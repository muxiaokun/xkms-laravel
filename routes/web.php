...
Route::get('Minify/{type}/{files}', function ($type, $files) {
    $content      = '';
    $content_type = '';
    switch ($type) {
        case 'css':
    ...
    foreach (explode(',', $files) as $file) {
        ...
        if (!$cache_content) {
            ...
            cache([$cache_name => $cache_content], 1);
        }
    ...
    $PageWasUpdated = ($request->header('If-Modified-Since') and strtotime($request->header('If-Modified-Since')) == $LastChangeTime);
    $DoIDsMatch     = ($request->hasHeader('If-None-Match') and $request->header('If-None-Match') == $HashID);
